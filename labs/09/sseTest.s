...
.LFB697:
	...
	#First loop start, Load aligned 
	movupd	xmm2, XMMWORD PTR [rsp+16]
	movapd	xmm5, XMMWORD PTR .LC5[rip]
	mulpd	xmm5, xmm2 #loop multiplication start
	movsd	QWORD PTR [rsp+32], xmm1 
	movupd	xmm4, XMMWORD PTR [rsp+48]
	movsd	QWORD PTR [rsp+40], xmm7
	movupd	xmm3, XMMWORD PTR [rsp+64]
	addpd	xmm4, xmm5 #adding(?)
	xorpd	xmm5, xmm5
	#First loop End, second loop start
	movsd	QWORD PTR [rsp+8], xmm6
	mulpd	xmm2, xmm5 #second loop multiplication
	addpd	xmm3, xmm2 #second add
	#second loop end, Third and fourth loop start
	movupd	xmm2, XMMWORD PTR [rsp+32]
	mulpd	xmm5, xmm2 #third loop multiplication
	mulpd	xmm2, XMMWORD PTR .LC6[rip] #fourth loop multiplication
	addpd	xmm4, xmm5 #third add
	addpd	xmm3, xmm2 #fourth add
	#Third and fourth loop end
