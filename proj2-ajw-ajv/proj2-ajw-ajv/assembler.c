...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        char* t = strtok(buf, IGNORE_CHARS);
        if (!t) {
            continue;
        }
        int label = add_if_label(input_line, t, byte_offset, symtbl);
        if (label == 1) {
            t = strtok(NULL, IGNORE_CHARS);
            if (!t) {
                continue;
            }
        } else if (label == -1) {
        ...
        int parsing_args = parse_args(input_line, args, &num_args);
        if (parsing_args == -1) {
            ret_code = -1;
            continue;
        }
        ...
        unsigned int lines_written = write_pass_one(output, t, args, num_args);
        if (lines_written == 0) {
            raise_inst_error(input_line, t, args, num_args);
            ret_code = -1;
        }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        /* Use translate_inst() to translate the instruction and write to output file.
           ...
           raise_inst_error(). */
        int ret_val = translate_inst(output, t, args, num_args, byte_offset, symtbl, reltbl);
        if (ret_val == -1) {
            raise_inst_error(input_line, t, args, num_args);
            ret_code = -1;
        }
